"""
Experiment 3: Dual-Scale ParGo Training Script
================================================
Based on the single-scale ParGo training script, extended for dual-scale.

Key differences from single-scale:
  - mm_projector_type = "dual_scale_pargo"
  - proj_out_num = 288 (256 low + 32 high)
  - Vision tower must output both penultimate and final layer features

This script supports ALL projector types:
  ...
  - "single_scale_pargo" -> 32 tokens (Experiment 2)
  - "dual_scale_pargo"  -> 288 tokens (Experiment 3)

Usage:
  Stage 2 (Projector Pretraining):
    bash scripts/exp3_stage2_pretrain.sh
  Stage 3 (LoRA Fine-tuning):
    bash scripts/exp3_stage3_finetune.sh
"""
...
class ModelArguments:
    ...
    mm_projector_type: Optional[str] = field(default="dual_scale_pargo")
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    bf16: bool = True
    output_dir: str = "./output/exp3-dual-scale-pargo"
...
def main():
    ...
    rank0_print("=" * 60)
    rank0_print("EXPERIMENT 3: DUAL-SCALE PARGO")
    rank0_print("=" * 60)
    ...
    if model_args.mm_projector_type is not None:
        ...
        elif model_args.mm_projector_type == "single_scale_pargo":
            model_args.proj_out_num = 32
        elif model_args.mm_projector_type == "dual_scale_pargo":
            model_args.proj_out_num = 288  # 256 low + 32 high
        else:
