...
def verify_modified_pargo_config(model_args, model):
    """Verify that modified_pargo is configured correctly."""
    if model_args.mm_projector_type != "modified_pargo":
        return  # Not using modified_pargo, skip checks
    
    rank0_print("\n" + "=" * 60)
    rank0_print("MODIFIED PARGO CONFIGURATION CHECK")
    rank0_print("=" * 60)
    
    # Check model args
    rank0_print(f"✓ Projector type: {model_args.mm_projector_type}")
    rank0_print(f"✓ Low-level tokens: {model_args.n_low_tokens}")
    rank0_print(f"✓ High-level tokens: {model_args.n_high_tokens}")
    rank0_print(f"✓ Total output tokens: {model_args.proj_out_num}")
    rank0_print(f"  Expected: {model_args.n_low_tokens + model_args.n_high_tokens}")
    
    # Verify token count matches
    expected_tokens = model_args.n_low_tokens + model_args.n_high_tokens
    if model_args.proj_out_num != expected_tokens:
        rank0_print(f"⚠️  WARNING: Token mismatch! Expected {expected_tokens}, got {model_args.proj_out_num}")
    else:
        rank0_print(f"✓ Token count correct: {model_args.proj_out_num}")
    
    # Check model config
    rank0_print(f"✓ Vision hidden size: {model.config.vision_hidden_size}")
    rank0_print(f"✓ LLM hidden size: {model.config.llm_hidden_size}")
    rank0_print(f"✓ Use positional embedding: {model.config.use_positional_embedding}")
    
    # Check projector exists
    if hasattr(model.get_model(), 'mm_projector'):
        projector = model.get_model().mm_projector
        rank0_print(f"✓ Projector class: {type(projector).__name__}")
        if hasattr(projector, 'proj_out_num'):
            rank0_print(f"✓ Projector output tokens: {projector.proj_out_num}")
...
class ModelArguments:
    ...
    # projector
    mm_projector_type: Optional[str] = field(default="modified_pargo")
    ...
    n_low_tokens: int = field(default=144, metadata={"help": "Number of low-level tokens"})
    n_high_tokens: int = field(default=32, metadata={"help": "Number of high-level tokens"})
    low_level_hidden_size: int = field(default=384, metadata={"help": "DCFormer penultimate layer dimension"})
    pargo_num_layers: int = field(default=6, metadata={"help": "Number of ParGo layers per scale"})
    use_cross_scale_attention: bool = field(default=False, metadata={"help": "Enable cross-scale attention"})
    
    use_positional_embedding: bool = field(default=True)
    # pos_embed_dim: int = field(default=3) #made modifications for new positional embeddings
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    bf16: bool = True
    output_dir: str = "./output/Med3DVLM-modified-pargo-stage1"  # Changed output name
    num_train_epochs: float = 5  # ← CHANGED from 1 to 5 for Stage 1!
    per_device_train_batch_size: int = 8  # ← CHANGED: Increased from 1
    per_device_eval_batch_size: int = 1
    gradient_accumulation_steps: int = 4  # ← CHANGED: For effective batch size of 32
    ...
    learning_rate: float = 5e-5  # ← CHANGED from 1e-4 to 5e-5!
    weight_decay: float = 0.0  # ← KEEP at 0.0 (important!)
    warmup_ratio: float = 0.05  # ← CHANGED from 0.03 to 0.05
    lr_scheduler_type: str = "cosine"
    logging_steps: float = 10
    gradient_checkpointing: bool = False
    max_grad_norm: float = 1.0  # ← NEW: Add gradient clipping!
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer, output_dir: str, tokenizer=None):
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        ...
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            ...
            rank0_print("Saving complete model with all components (DCFormer + Modified_ParGo + Qwen)...")
...
def main():
    ...
    if model_args.mm_projector_type is not None:
        ...
        elif model_args.mm_projector_type == "modified_pargo":
            # Auto-calculate from multi-scale tokens
            model_args.proj_out_num = model_args.n_low_tokens + model_args.n_high_tokens  # 144 + 32 = 176
            model_args.vision_width = 768  # DCFormer final layer dimension
            rank0_print(f"Modified ParGo: {model_args.n_low_tokens} low + {model_args.n_high_tokens} high = {model_args.proj_out_num} total tokens")
        else:
    ...
    if model_args.mm_projector_type == "modified_pargo":
        model.config.n_low_tokens = model_args.n_low_tokens
        model.config.n_high_tokens = model_args.n_high_tokens
        model.config.low_level_hidden_size = model_args.low_level_hidden_size
        model.config.pargo_num_layers = model_args.pargo_num_layers
        model.config.use_cross_scale_attention = model_args.use_cross_scale_attention
        rank0_print(f"Modified ParGo config: low_tokens={model.config.n_low_tokens}, high_tokens={model.config.n_high_tokens}")

    # model.config.pos_embed_dim = model_args.pos_embed_dim #made modifications for new positional embeddings
    ...
    if model_args.vision_tower is not None:
        ...
        verify_modified_pargo_config(model_args, model)
