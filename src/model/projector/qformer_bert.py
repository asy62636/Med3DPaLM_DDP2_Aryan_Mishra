...
class BertEmbeddings(nn.Module):
    ...
    def forward(
        ...
        if input_ids is not None:
            ...
            # position embeddings
            position_ids = self.position_ids[ :,  : query_embeds.shape[1]].clone()
            position_embeddings = self.position_embeddings(position_ids).repeat(query_embeds.shape[0], 1, 1 )
...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        # cross_attention between query and images.
        if is_cross_attention:
        ...
        query_layer = self.transpose_for_scores(mixed_query_layer)      # transpose to 16 heads
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        if query_length > 0:
            ...
            # corss_attention between image query and q-former queries
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        #     )
        if is_decoder:
            # use embedding-based shape if input_ids is None (query-only path)
            dec_input_shape = input_ids.shape if input_ids is not None else (batch_size, attention_mask.shape[1])
            extended_attention_mask = self.get_extended_attention_mask(
                attention_mask,
                dec_input_shape,
                device,
            ...
            print("Extended attention mask handled yayyyyy!")
        else:
            extended_attention_mask = self.get_extended_attention_mask(
