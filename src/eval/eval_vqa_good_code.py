...
All projector-specific settings are read from config.json automatically.

Usage:
  # Open-ended
  python eval_vqa.py --model_path ./output/exp1-baseline-complete \
      --output_dir ./output/eval_vqa_exp1 --test_size 100

  # Closed-ended
  python eval_vqa.py --model_path ./output/exp1-baseline-complete \
      --output_dir ./output/eval_vqa_exp1 --close_ended --test_size 100
"""
...
def parse_args(args=None):
    ...
    parser.add_argument("--close_ended", action="store_true", default=False,
                        help="Run closed-ended VQA (default: open-ended)")
...
def load_complete_model(model_path, device):
    ...
    All projector settings are read from config.json — works for all 3 experiments.
    ...
    # 3. Load base model weights from safetensors
    # IMPORTANT: device_map={"": device} puts everything on ONE device.
    # device_map="auto" spreads across GPUs, causing device-mismatch errors
    # when initialize_vision_modules later creates new modules on CPU.
    ...
    gm = model.get_model()
    mm_keys = [k for k in model.state_dict().keys() if "mm_projector" in k]
    print(f"\n  mm_projector keys in state_dict: {len(mm_keys)}")

    if len(mm_keys) > 0 and getattr(gm, "mm_projector", None) is not None:
        print("  ✓ Projector loaded automatically by from_pretrained")
        model.eval()
        print(f"\n  ✓ Model ready | projector: {mm_projector_type} | tokens: {proj_out_num}")
        print("=" * 60)
        return model, tokenizer, config_dict

    # 5. Projector not auto-loaded — initialize then load weights manually
    ...
    model_args = SimpleNamespace(
        ...
        # ParGo-specific (Exp 2 & 3) — safely ignored if not a ParGo projector
    ...
    # Mirror onto model.config so build_mm_projector can read them
    for attr in [
    ...
    # Move newly created modules to same device/dtype as the LLM
    model_dtype  = next(model.parameters()).dtype
    model_device = next(model.parameters()).device
    print(f"\n  Moving vision modules to {model_device} ({model_dtype})...")
...
def compute_open_metrics(preds, labels):
    """Compute BLEU, ROUGE1, METEOR, BERTScore for one sample."""
    result = {}
    try:
        result["bleu"] = float(
    ...
    try:
        result["rouge1"] = float(
            rouge.compute(predictions=preds, references=labels, rouge_types=["rouge1"]).get("rouge1", 0.0)
        )
    except Exception:
        result["rouge1"] = 0.0
    try:
...
def generate_answer(model, tokenizer, input_ids, attention_mask, images, args, device):
    """Run model.generate and return decoded string."""
    gen_kwargs = dict(
        inputs=input_ids,
...
def eval_closed(model, tokenizer, dataloader, args, device, output_dir):
    ...
    with open(output_path, mode="w", newline="") as outfile:
        ...
        for idx, sample in enumerate(tqdm(dataloader, desc="Closed-Ended")):
            ...
            try:
                pred = generate_answer(model, tokenizer, input_ids, attn_mask, image, args, device)
...
def main():
    ...
    # Load model (reads all settings from config.json automatically)
    model, tokenizer, config_dict = load_complete_model(args.model_path, device)

    # Stash projector type on args so eval loops can include it in summaries
