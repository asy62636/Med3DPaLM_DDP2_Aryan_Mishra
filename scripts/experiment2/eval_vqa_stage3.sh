...
MODEL_PATH=${BASE}/output/exp2-single-pargo-stage3-complete
VQA_DATA=${BASE}/data/M3D-VQA/M3D_VQA_test.csv
LOG_DIR=${BASE}/training_logs

echo "=== Exp2 Stage3: Open-ended VQA ==="
PYTHONPATH=. python ${BASE}/src/eval/eval_vqa.py \
    --model_path     ${MODEL_PATH} \
    --output_dir     ${BASE}/output/eval_exp2_stage3_vqa/open \
    --vqa_data_test_path ${VQA_DATA} \
    --data_root      ${BASE}/data \
    --max_length     512 \
    --max_new_tokens 256 \
    > ${LOG_DIR}/eval_vqa_exp2_stage3_open.txt 2>&1

echo "Open-ended done at: $(date)"

echo "=== Exp2 Stage3: Closed-ended VQA ==="
PYTHONPATH=. python ${BASE}/src/eval/eval_vqa_good_code_stage_3.py \
    --model_path     ${MODEL_PATH} \
    --output_dir     ${BASE}/output/eval_exp2_stage3_vqa/closed \
    --vqa_data_test_path ${VQA_DATA} \
    --data_root      ${BASE}/data \
    --max_length     512 \
    --max_new_tokens 256 \
