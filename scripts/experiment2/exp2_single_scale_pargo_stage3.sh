#!/bin/bash
# ==============================================================================
# EXPERIMENT 2 — Stage 3: Single-Scale ParGo LoRA Fine-tuning
# ==============================================================================
# All paths, GPU count, batch sizes, and hyperparameters mirror expt2.sh.
...
echo "============================================"
echo "Experiment 2 — Stage 3: Single-Scale ParGo LoRA Fine-tuning"
echo "  Projector: single_scale_pargo (loaded from Stage 2)"
echo "  LoRA: r=16, alpha=32, dropout=0.05"
echo "  vision_select_layer: -1 (matches Stage 2)"
echo "  Effective batch: 24 (4 x 2 x 3 GPUs)"
echo "  LR: 5e-5, Epochs: 5"
echo "  model_max_length: 2048"
echo "============================================"

# --- Paths (identical to expt2.sh) ---
VISION_ENCODER="./output2/DCFormer_SigLIP/pretrained_ViT.bin"
STAGE2_PROJECTOR="./output/exp2-single-pargo/stage2/mm_projector.bin"
OUTPUT_DIR="./output/exp2-single-pargo/stage3"
DATA_ROOT="./data"
...
if [ ! -f "$STAGE2_PROJECTOR" ]; then
    ...
    echo "      --checkpoint ./output/exp2-single-pargo/stage2/checkpoint-97944 \\"
...
# --- Launch ---
PYTHONPATH=. deepspeed --num_gpus=4 $TRAIN_SCRIPT \
    --deepspeed $DS_CONFIG \
    --wb_name EXP2_SINGLE_PARGO_STAGE3_LORA \
    ...
    --seed 42 \
    > training_logs/exp2_single_pargo_stage3_lora.txt 2>&1

echo ""
echo "============================================"
echo "Experiment 2 — Stage 3 complete!"
echo "============================================"
echo "Check log:  tail -100 training_logs/exp2_single_pargo_stage3_lora.txt"
