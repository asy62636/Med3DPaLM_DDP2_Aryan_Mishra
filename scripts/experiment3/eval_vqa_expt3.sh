...
# ------------------------------------------------------------------------------
echo "=== Exp 3: Open-ended VQA ===" | tee ${LOG_DIR}/exp3_vqa_open.txt
PYTHONPATH=. python /home/medal/ankit_k/Med3DVLM_and_Pargo/experiments/src/eval/eval_vqa_good_code.py \
    --model_path   ${MODEL_PATH} \
    --output_dir   ./output/eval_exp3_vqa/open \
    --vqa_data_test_path ${VQA_DATA} \
    --max_length   512 \
    --max_new_tokens 256 \
    --test_size    100 \
    >> ${LOG_DIR}/exp3_vqa_open.txt 2>&1

echo "Open-ended done. Log: ${LOG_DIR}/exp3_vqa_open.txt"

# ------------------------------------------------------------------------------
# Closed-ended VQA
# ------------------------------------------------------------------------------
echo "=== Exp 3: Closed-ended VQA ===" | tee ${LOG_DIR}/exp3_vqa_close.txt
PYTHONPATH=. python /home/medal/ankit_k/Med3DVLM_and_Pargo/experiments/src/eval/eval_vqa_good_code.py \
    --model_path   ${MODEL_PATH} \
    --output_dir   ./output/eval_exp3_vqa/closed \
    --vqa_data_test_path ${VQA_DATA} \
    --max_length   512 \
    --max_new_tokens 256 \
    --close_ended \
    --test_size    100 \
    >> ${LOG_DIR}/exp3_vqa_close.txt 2>&1

echo "Closed-ended done. Log: ${LOG_DIR}/exp3_vqa_close.txt"
echo "=== Exp 3 VQA evaluation complete ==="